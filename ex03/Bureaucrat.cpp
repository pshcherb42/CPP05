...
// NEW: Pointer-based overloaded functions for NULL safety
void Bureaucrat::signForm(AForm* f) {
    if (!f) {
        std::cout << _name << " cannot sign - NULL form pointer" << std::endl;
        return;
    }
    signForm(*f);  // Call the reference version
}

void Bureaucrat::executeForm(AForm const * form) const {
    if (!form) {
        std::cout << _name << " cannot execute - NULL form pointer" << std::endl;
        return;
    }
    executeForm(*form);  // Call the reference version
}